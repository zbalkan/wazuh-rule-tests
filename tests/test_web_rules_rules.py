...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            ...
            '31106',
            6,
            id='a_web_attack_returned_code_200_success',
        ),
        pytest.param(
            r"""2015-07-28 15:07:26 1.2.3.4 GET /QOsa/Browser/Default.aspx UISessionId=SN1234123&DeviceId=SN12312232SHARP+MX-4111N 80 - 31.3.3.7 OpenSystems/1.0;+product-family="85";+product-version="123ER123" 302 0 0 624""",
            'web-accesslog-iis-default',
            '31108',
            0,
            id='a_web_page_returned_code_302_code',
        ),
        pytest.param(
            r"""2015-03-11 21:59:09 1.2.3.4 GET /console/faces/com_sun_web_ui/jsp/version/version_30.jsp - 80 - 31.3.3.7 Sun+Web+Console+Fingerprinter/7.15 - 404 0 2 0""",
            'web-accesslog-iis-default',
            '31101',
            5,
            id='a_web_page_returned_code_404_not_found',
        ),
        pytest.param(
            ...
            '31104',
            6,
            id='a_web_attacked_returned_code_404',
        ),
        pytest.param(
            r'''Jan 11 10:13:05 web01 nginx: ::ffff:202.194.15.192 190.7.138.180 - [18/Oct/2010:10:48:55 -0500] "GET //php-my-admin/config/config.inc.php?p=phpinfo(); HTTP/1.1" 404 345 "-"  "Mozilla/4.0 (compatible; MSIE 6.0; Windows 98)"''',
            'web-accesslog',
            '31101',
            5,
            id='a_web_page_returned_code_404_not_found_syslog_format',
        ),
        pytest.param(
            r'''Jan 11 10:13:05 web01 nginx: 10.10.10.11 10.10.10.12 - - [10/Apr/2017:13:18:05 -0700] "GET /injection/%0d%0aSet-Cookie HTTP/1.1" 404 271 "-" "Mozilla/5.0 (Windows NT 6.1; WOW64; rv:22.0) Gecko/20100101 Firefox/22.0"''',
            'web-accesslog',
            '31104',
            6,
            id='a_web_attacked_returned_code_404_syslog_format',
        ),
        pytest.param(
            r'''Jan 11 10:13:05 web01 apache: 10.11.12.13 - - - [27/Mar/2017:13:40:40 -0700] "GET /modules.php?name=Search&type=stories&query=qualys&category=-1%20&categ=%20and%201=2%20UNION%20SELECT%200,0,aid,pwd,0,0,0,0,0,0%20from%20nuke_authors/* HTTP/1.0" 404 982 "-" "-"''',
            'web-accesslog',
            '31103',
            7,
            id='sql_injection_attempt_syslog_format',
        ),
        pytest.param(
            r'''www.example.com:80 1.1.1.1 - - [12/Mar/2019:10:09:09 +0000] "GET /example?dir=asc&order=if(now()=sysdate()%2csleep(19.406)%2c0)/*'XOR(if(now()=sysdate()%2csleep(19.406)%2c0))OR'\"XOR(if(now()=sysdate()%2csleep(19.406)%2c0))OR\"*/&example=5101 HTTP/1.1" 200 10869 "https://www.example.com" "Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.21 (KHTML, like Gecko) Chrome/41.0.2228.0 Safari/537.21" "-" "XIeFRRmu6BzGhDFQJ1K@HwAAAAs"''',
            'web-accesslog',
            '31170',
            6,
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            ...
            '31106',
            6,
            id='not_a_web_attack_returned_code_200_success',
        ),
    ],
)
